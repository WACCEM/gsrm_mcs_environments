...
def extract_variable_statistics_from_masks(track_df, mask_data, variable_data, 
    ...
    This approach pre-aligns track times to model output times, then loads masks for 
    small batches of unique times (batch_size at a time), processes all tracks at those 
    times, then moves to the next batch. This amortizes I/O overhead by reusing loaded 
    mask time slices across multiple tracks.
    ...
    """
    print(f"\nExtracting {variable_name} statistics using track masks (pre-aligned approach)...")
    sys.stdout.flush()
    
    total_points = len(track_df)
    print(f"  Processing {total_points} track-time combinations")
    
    # PRE-ALIGN: Round all track times to nearest model output time BEFORE batching
    # This eliminates duplicate time coordinates in batch selections
    freq_hours = int(model_freq.rstrip('H').rstrip('h'))
    track_df = track_df.copy()
    track_df['aligned_time'] = pd.to_datetime(track_df['base_time']).dt.round(f'{freq_hours}H')
    
    original_unique = track_df['base_time'].nunique()
    aligned_unique = track_df['aligned_time'].nunique()
    print(f"  Pre-alignment: {original_unique} original times → {aligned_unique} aligned times")
    
    # Get track start times for time offset calculation (use original base_time)
    track_start_times = track_df.groupby('track_id')['base_time'].first().to_dict()
    
    # Group by unique ALIGNED times for batched processing
    unique_times = sorted(track_df['aligned_time'].unique())
    n_unique_times = len(unique_times)
    
    print(f"  Unique aligned times: {n_unique_times}")
    ...
    for batch_idx in range(0, n_unique_times, batch_size):
        ...
        # Load mask and variable data for this batch of ALIGNED times
        # No method='nearest' needed since times are already aligned!
        ...
        # Get all tracks at these ALIGNED times
        df_batch = track_df[track_df['aligned_time'].isin(batch_times)]
        ...
        for idx, row in df_batch.iterrows():
            ...
            try:
                ...
                # Use aligned_time for extraction
                stats = extract_statistics_from_mask(variable_batch, mask_cells, aligned_time)
                ...
                # Store result with aligned_time as data_time
                result = {
                    'track_id': track_id,
                    'time_idx': time_idx,
                    'data_time': aligned_time,  # Use aligned time for data extraction
